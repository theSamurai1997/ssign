...
def main():
    ...
    if not seqs:
        ...
        with open(args.output, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["locus_tag", "ortholog_group", "og_n_members", "og_mean_pident"])
            writer.writeheader()
        return
    ...
    if len(seqs) < 2:
        ...
        with open(args.output, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["locus_tag", "ortholog_group", "og_n_members", "og_mean_pident"])
            writer.writeheader()
            writer.writerow({"locus_tag": pid, "ortholog_group": "OG_001", "og_n_members": 1, "og_mean_pident": 100.0})
        return

    # Run all-vs-all BLASTp
    hits = run_local_blast(args.substrates_fasta, args.min_pident, args.min_qcov, args.evalue)
    ...
    if hits is None:
        ...
        with open(args.output, "w", newline="") as f:
            ...
            writer.writeheader()
            for pid in seqs:
                writer.writerow({"locus_tag": pid, "ortholog_group": "", "og_n_members": 0, "og_mean_pident": 0})
        return
